...
| ?n             | the next instruction is executed only if the case at the cursor is equal to n (n must be between 0 and 255 included)
| ?id            | the next instruction is executed only if the case at the cursor is equal to n where n is value at the case pointed the reference id
| !id            | the next instruction is executed if the cursor is at the reference id, the machine crash if the label does not exist
